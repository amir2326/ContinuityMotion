...
final class Prefs {
    ...
    static int handoffMs(Context c) { return prefs(c).getInt("handoffMs", 190); }
    ...
    static float coverAspect(Context c) { return prefs(c).getFloat("coverAspect", 0.41f); }
    ...
    static void setCoverAspect(Context c, float v) {
        prefs(c).edit().putFloat("coverAspect", MotionMath.clamp(v, .30f, .72f)).apply();
    }
